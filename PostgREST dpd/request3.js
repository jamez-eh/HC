		...
		for (dataIndex; dataIndex < maxDataIndex; dataIndex++) {
			...
			*/
			...
			for(var i = 0; i < obj.active_ingredients.length; i++) {
				activeText += obj.active_ingredients[i].ingredient + " (" + obj.active_ingredients[i].strength + " " + obj.active_ingredients[i].strength_unit + ")";

				if (i != obj.active_ingredients.length - 1) {
					activeText += ", "
				}
			}

			activeCell.innerHTML = activeText;
