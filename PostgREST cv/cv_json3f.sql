CREATE MATERIALIZED VIEW cv_json3f AS(
	SELECT cv.report_id, 
	cv.cv_report,
	to_tsvector('français'::regconfig, (cv.cv_report)::text) AS search
	FROM( 
		SELECT r.report_id, jsonb_strip_nulls(jsonb_build_object(
			...
			'report_type', r.report_type_fr,
			'gender_code', r.gender_code,
			'gender', NULLIF(r.gender_fr, ''::text),
			'age', r.age,
			'age_y', r.age_y,
			'age_unit', NULLIF(r.age_unit_fr, ''::text),
			'outcome_code', r.outcome_code,
			'outcome', r.outcome_fr,
			'weight', r.weight,
			'weight_unit', NULLIF(r.weight_unit_fr, ''::text),
			'height', r.height,
			'height_unit', NULLIF(r.height_unit_fr, ''::text),
			'seriousness_code', r.seriousness_code,
			'seriousness', r.seriousness_fr,
			'death', r.death,
			...
			'reporter_type', NULLIF(r.reporter_type_fr, ''::text),
			'source_code', r.source_code,
			'source', NULLIF(r.source_fr, ''::text),
			'report_links', (
				SELECT jsonb_agg(to_jsonb(l.*)) 
				AS jsonb_agg 
				FROM(
					SELECT cv_report_links.report_link_id, 
						NULLIF(cv_report_links.record_type_fr, ''::text) AS record_type, 
			...
			'report_drugs', (
				...
				FROM(
					SELECT cv_report_drug.report_drug_id, 
						...
						(cv_report_drug.druginvolv_fr) AS druginvolv,
						(cv_report_drug.dosageform_fr) AS dosageform,
			...
			'report_drug_indication', (
				...
				FROM(
					SELECT cv_report_drug_indication.report_drug_id,
						...
						NULLIF(cv_report_drug_indication.indication_name_fr, ''::text) AS indication_name
			...
			'reactions', (
				...
				FROM(
					SELECT cv_reactions.reaction_id,
						NULLIF(cv_reactions.pt_name_fr, ''::text) AS pt_name,
						NULLIF(cv_reactions.soc_name_fr, ''::text) AS soc_name,
